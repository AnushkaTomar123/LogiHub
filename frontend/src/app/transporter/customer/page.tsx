    ...
    return (
        <div 
            ...
            className={`flex items-center p-3 cursor-pointer transition-colors ${
                isActive ? 'bg-white dark:bg-background' : 'hover:bg-white dark:hover:bg-background'
            } border-b border-gray-700`}
        >
            <div className="relative w-12 h-12 mr-3">
                <div className="w-full h-full rounded-full bg-orange-500 flex items-center justify-center text-800 dark:text-800 dark:text-white text-lg font-bold">
                    {user.name.charAt(0)}
                </div>
                <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full ${user.status === 'Online' ? 'bg-green-500' : 'bg-gray-100 dark:bg-card'} border-2 border-[#1A1F26]`}></div>
            </div>

            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-800 dark:text-800 dark:text-white truncate">{user.name}</p>
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${roleClass} text-800 dark:text-800 dark:text-white`}>{user.role}</span>
                </div>
            ...
            <div className="flex flex-col items-end text-xs ml-2">
                ...
                {user.unreadCount > 0 && (
                    <span className={`w-4 h-4 mt-1 rounded-full flex items-center justify-center text-800 dark:text-800 dark:text-white ${unreadClass}`}>
                        {user.unreadCount}
                    </span>
                )}
            </div>
...
const ChatBubble: React.FC<{ message: Message }> = ({ message }) => {
    ...
    return (
        <div className={`flex ${isYou ? 'justify-end' : 'justify-start'} mb-4`}>
            <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl text-800 dark:text-800 dark:text-white shadow-md ${
                isYou ? 'bg-violet-600 rounded-br-none' : 'bg-white dark:bg-background rounded-tl-none'
            }`}>
...
const PlaceholderPanel: React.FC<{ section: ActiveSection }> = ({ section }) => {
    ...
    return (
        <div className="p-10 flex flex-col items-center justify-center h-full text-center bg-white dark:bg-background rounded-xlborder border-gray-50 dark:border-zinc-500">
            {React.createElement(iconMap[section], { size: 64, className: 'text-violet-500 mb-4' })}
            <h2 className="text-2xl font-bold text-800 dark:text-800 dark:text-white mb-2">{titleMap[section]}</h2>
...
export default function CustomerChats() {
  ...
  return (
    <div
        ...
        className={`min-h-screen bg-gray-50 dark:bg-background text-gray-800 dark:text-gray-100 transition-colors duration-300`}
    >
      <TransporterHeader />
          ...
          <button 
            ...
            className={`px-6 py-2 rounded-t-xl font-semibold transition-colors border-b-4 ${
              activeSection === tab.section 
                ? 'border-violet-600 bg-white dark:bg-background text-800 dark:text-800 dark:text-white' 
                : 'border-transparent text-gray-400 hover:text-800 dark:text-800 dark:text-white hover:border-gray-600'
            }`}
          >
            {tab.label}
          </button>
      ...
      <div className="p-6 pt-3 h-[calc(100vh-130px)]">
        ...
        {activeSection === 'chats' && (
            <div className="flex h-full gap-6">
                ...
                {showChatList && (
                    <div className={`flex flex-col ${isMobile ? 'w-full' : 'w-1/3 min-w-[300px]'} bg-white dark:bg-background rounded-xl shadow-lg overflow-hidden`}>
                        <div className="p-3 bg-gray-50 dark:bg-card flex flex-col justify-between border-b border-gray-700">
                            <h2 className="text-xl font-bold text-800 dark:text-800 dark:text-white mb-2">All chats</h2>
                            ...
                            <div className="relative w-full">
                                <input
                                    ...
                                    className="w-full p-2 pl-10 rounded-lg bg-white dark:bg-background text-800 dark:text-800 dark:text-whiteborder border-gray-50 dark:border-zinc-500 focus:ring-violet-500 focus:border-violet-500 text-sm"
                                />
                                <MdSearch size={20} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-300" />
                            </div>
                        </div>
                ...
                {showChatPanel && activeChat && (
                    <div className={`flex flex-col ${isMobile ? 'w-full absolute inset-0 bg-gray-50 dark:bg-card z-40' : 'w-2/3 flex-1'} bg-gray-50 dark:bg-card rounded-xl shadow-lgborder border-gray-50 dark:border-zinc-500`}>
                        
                        {/* Chat Header */}
                        <div className="p-4 bg-white dark:bg-background rounded-t-xl flex justify-between items-center border-b border-gray-700">
                          <div className="flex items-center">
                            {isMobile && (
                                <MdArrowBack onClick={() => setActiveChat(MOCK_CHAT_LIST[0])} size={24} className="mr-3 text-800 dark:text-800 dark:text-800 dark:text-white cursor-pointer"/>
                            )}
                            <div className="w-10 h-10 rounded-full bg-orange-500 flex items-center justify-center text-800 dark:text-800 dark:text-white text-md font-bold mr-3">
                                {activeChat.name.charAt(0)}
                            </div>
                            <div>
                              <h3 className="text-lg font-semibold text-800 dark:text-800 dark:text-white">{activeChat.name}</h3>
                          ...
                          <div className="flex gap-4 text-gray-400">
                            <MdCall size={24} className="hover:text-800 dark:text-800 dark:text-white cursor-pointer" onClick={() => handleCall('Audio')} title="Audio Call"/>
                            <MdVideocam size={24} className="hover:text-800 dark:text-800 dark:text-white cursor-pointer" onClick={() => handleCall('Video')} title="Video Call"/>
                            <MdMoreVert size={24} className="hover:text-800 dark:text-800 dark:text-white cursor-pointer" />
                          </div>
                        </div>
                        ...
                        <div className="flex-1 p-6 overflow-y-auto space-y-2">
                            <div className="text-center my-4">
                                <span className="text-xs text-gray-400 bg-gray-50 dark:bg-card px-3 py-1 rounded-fullborder border-gray-50 dark:border-zinc-500">
                                    Today 
                                </span>
                            </div>
                            {messages.map(msg => (
                        ...
                        <form onSubmit={handleSendMessage} className="p-4 bg-white dark:bg-background flex items-center gap-3 border-t border-gray-700">
                          <input
                            type="text"
                            placeholder="Type a message..."
                            value={newMessage}
                            onChange={(e) => setNewMessage(e.target.value)}
                            className="flex-1 p-3 rounded-xl bg-gray-50 dark:bg-card text-800 dark:text-800 dark:text-whiteborder border-gray-50 dark:border-zinc-500 focus:ring-violet-500 focus:border-violet-500"
                          />
                          <button type="submit" className="p-3 bg-violet-600 rounded-full hover:bg-violet-700 transition">
                            <MdSend size={24} className="text-800 dark:text-800 dark:text-white" />
                          </button>
                        </form>
                    </div>
                )}
